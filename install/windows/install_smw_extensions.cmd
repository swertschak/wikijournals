...
echo - Install Semantic Mediawiki Extensions - >> %PROJECTDIR%\install\windows\wikijournals.log
...
if %ERRORLEVEL% NEQ 0 (
echo Cloning Semantic Drilldown repository failed >> %PROJECTDIR%\install\windows\wikijournals.log
exit 1
)

git clone https://gerrit.wikimedia.org/r/p/mediawiki/extensions/PageForms
if %ERRORLEVEL% NEQ 0 (
echo Cloning PageForms repository failed >> %PROJECTDIR%\install\windows\wikijournals.log
exit 1
)

git clone https://gerrit.wikimedia.org/r/p/mediawiki/extensions/SemanticInternalObjects.git
if %ERRORLEVEL% NEQ 0 (
echo Cloning Semantic Internal Objects repository failed >> %PROJECTDIR%\install\windows\wikijournals.log
exit 1
)
...
if %ERRORLEVEL% NEQ 0 (
echo Install Maps using composer failed >> %PROJECTDIR%\install\windows\wikijournals.log
exit 1
)

call composer require mediawiki/semantic-result-formats "3.1.0"
if %ERRORLEVEL% NEQ 0 (
echo Install Semantic Result Formats using composer failed >> %PROJECTDIR%\install\windows\wikijournals.log
exit 1
)

call composer require mediawiki/semantic-compound-queries "2.1.0"
if %ERRORLEVEL% NEQ 0 (
echo Install Semantic Compound Queries using composer failed >> %PROJECTDIR%\install\windows\wikijournals.log
exit 1
)

type LocalSettings.php %PROJECTDIR%\config\configSMWExtensions.txt > LocalSettings.bak
if %ERRORLEVEL% NEQ 0 (
echo Appending SMW Extensions Config to LocalSettings.php failed >> %PROJECTDIR%\install\windows\wikijournals.log
exit 1
)
...
echo Semantic Mediawiki Extensions installed successful >> %PROJECTDIR%\install\windows\wikijournals.log